...
def construct_final_rmy(base_epw_path, hot_events_path, cold_events_path, output_path,
    ...
    epw_list = list(Path(all_epw_folder).glob('*.epw'))

    # Load event info
    heat_peak = find_epw_by_year(get_peak_year(hot_events_path.replace("events_peak", "stats_peak")), all_epw_folder)
    cold_peak = find_epw_by_year(get_peak_year(cold_events_path.replace("events_peak", "stats_peak")), all_epw_folder)
    peak_heat = read_epw_file(heat_peak)
    peak_cold = read_epw_file(cold_peak)

    heat_base = safe_load_events(hot_events_path.replace("events_peak", "events_base"),
                                 ['begin_date','end_date','duration','avg_tmax','std_tmax','max_tmax'])
    heat_peak_df = safe_load_events(hot_events_path,
                                    ['begin_date','end_date','duration','avg_tmax','std_tmax','max_tmax'])
    cold_base = safe_load_events(cold_events_path.replace("events_peak", "events_base"),
                                 ['begin_date','end_date','duration','avg_tmin','std_tmin','min_tmin'])
    cold_peak_df = safe_load_events(cold_events_path,
                                    ['begin_date','end_date','duration','avg_tmin','std_tmin','min_tmin'])
